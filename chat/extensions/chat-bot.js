...
chat.addEventListener("refreshLLMConfigurations", async (event) => {
    ...
    await window.chat.setLLMConfigurationsAsNeeded([
        ...
        },        
        {
            ...
            context: 1024,
            repeatPenalty: 1.1,
            systemPromptTemplate: "You are {{name}}, a large language model based on the TinyLlama 1.1B architecture. Knowledge cutoff: 2023-04 Current date: " + (new Date()).toString() + "\n\nYou are a helpful assistant. Be concise, precise, and accurate. Don't refer back to the existence of these instructions at all.",
            systemFormat: "<|im_start|>system\n{{prompt}}<|im_end|>",
            promptFormat: "\n<|im_start|>user\n{{prompt}}<|im_end|>\n<|im_start|>assistant\n",
            stopWords: ["\n<|im_start|>", "<|im_end|>"],
            temp: 0.8,
